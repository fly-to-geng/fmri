...
定义路径和加载数据
"""
root = r'D:\FMRI_ROOT\YIYU\CONN\conn_project03\meresult'
data_root = os.path.join(root,'data')
save_root = os.path.join(root,'class_result')
# 功能连接特征
features = r'features_from_fc_aal_first_level.mat'
features_name = r'features_from_fc_aal_first_level_vs_names.mat'
y_path = r'y.npy'
features = sio.loadmat(os.path.join(data_root,features))['subjects_features_mat']
...
Ks = np.linspace(1,X.shape[1],X.shape[1]).astype(int)
...
# 选出来特征选择 47 个， 分类器选择线性LR， 计算特征排序
# ==========================================================================
svc_linear =  SVC(C=1,kernel="linear")
cv = LeavePOut(y.shape[0], p=1)
sk_f = SelectKBest(my_advance,k=16)
score_linear_svc = []
feaure_indexs = []
...
net_f_acc = np.mean(score_linear_svc)
print net_f_acc
...
#p = (np.sum(random_score_list>net_f_acc) + 1 ) / float(k+1)
#print "分类显著性："
#print p
...
# 写入excel, 保存结果
save_root = r'D:\FMRI_ROOT\code_for_fmri\YIYU\7'
import xlwt
os.chdir(save_root)
workbook = xlwt.Workbook(encoding = 'ascii')
worksheet = workbook.add_sheet('region_weight_net_f')
...
worksheet = workbook.add_sheet('connection_weight_net_f')
for i in range(len(feature_sorted)):
...
workbook.save(r'classi_weight_func_net.xls')   
...
#node_size_end = [x*100 for x in node_size ] 

"""
写入脑区数据，for  brainnetviewer
"""
str2 = ''   
...
file_object = open(os.path.join(save_root,r'regions_func_net.node'), 'w')#######################################
...
#    str2 = str2 + '\n'
#
#file_object = open(os.path.join(save_root,r'regions_func_net_f.txt'), 'w')#######################################
#file_object.write(str2)
#file_object.close( )
...
#os.chdir(save_root)
#workbook = xlwt.Workbook(encoding = 'ascii')
#worksheet = workbook.add_sheet('plot_region_weigh_net_f')##########################################
...
#      
#workbook.save(r'net_f_selected_regions.xls') #########################################
...
#    strs = strs + '\n'
#
#file_object = open(os.path.join(save_root,r'nef_f_regions_links.txt'), 'w')  ####################
...
file_object = open(os.path.join(save_root,r'net_regions_links.edge'), 'w')  ####################
file_object.write(strs)
file_object.close( ) 
