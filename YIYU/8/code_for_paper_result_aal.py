...
for i in range(2,118):
    ...
    network_index.append(  table.cell(i,2).value )
    #network_names.append( table.cell(i,3).value )
#######################################################################    
    
import xlwt
os.chdir( r'D:\FMRI_ROOT\YIYU\Result\8')
...
for i in range(len(regions_sorted)):
    ...
    worksheet.write(i, 2, label = net1_index)
...
for i in range(len(regions_sorted2)):
    ...
    worksheet.write(i, 2, label = net2_index)
...
for i in range(len(feature_sorted)):
    ...
    worksheet.write(i, 3, label = network_index[index1])
    worksheet.write(i, 4, label = network_index[index2]) 
    
workbook.save(r'classi_weight_func_aal.xls') ##############################   

#################################################################################
"""
为brainnetviewer 生成数据
"""
