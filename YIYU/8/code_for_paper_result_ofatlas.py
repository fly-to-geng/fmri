...
Ks = np.linspace(10,2000,10).astype(int)
#Ks = np.linspace(1560,1610,25).astype(int)
...
# 选出来特征选择 1570个， 分类器选择线性linear_svm， 计算特征排序  准确率80.85%

cv = LeavePOut(y.shape[0], p=1)
sk_f = SelectKBest(my_advance,k=231)
score_linear_svc = []
feaure_indexs = []
...
workbook.save(r'classi_weight_func_ha.xls') ##############################   

#################################################################################

# 得到脑区的中心点的坐标信息，写入文件画图用
import xlrd
aal_file = r'D:\FMRI_ROOT\YIYU\ROIs\fsl-harvard-oxford-atlases\AAL.xlsx'
