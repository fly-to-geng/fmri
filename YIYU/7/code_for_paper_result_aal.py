...
X = features[:,end_mask]
##直接去一个T检验的值看看是多少
#pvalues1 = stats.ttest_1samp(features,0.0,axis=0)
#p1 = pvalues1[1]
#deleted_mask1 = p1<p # 要删除的连接
#print np.sum(deleted_mask1)
#
#print np.sum(np.logical_and(end_mask,deleted_mask1))
...
Ks = np.linspace(870,920,21).astype(int)
...
# 选出来特征选择 882个， 分类器选择线性linear_svm， 计算特征排序  准确率85.106%

cv = LeavePOut(y.shape[0], p=1)
sk_f = SelectKBest(my_advance,k=882)
score_linear_svc = []
feaure_indexs = []
...
acc_aal_class=np.mean(score_linear_svc)
# 准确率 70.21%
...
#p = (np.sum(random_score_list>acc_aal_class) + 1 ) / float(k+1)
#print "分类显著性："
#print p
...
max_weight = max(regions_weight.values())
"""
...
"""
特征排序和脑区权重排序写入Excel,这里加入所属的静息态网络
"""
#############################################################
...
for i in range(2,118):
    ...
    network_index.append(  table.cell(i,3).value )
    #network_names.append( table.cell(i,3).value )
#######################################################################    
    
import xlwt
os.chdir( r'D:\FMRI_ROOT\YIYU\Result\7')
workbook = xlwt.Workbook(encoding = 'ascii')
worksheet = workbook.add_sheet('region_weight_aal')#####################################
for i in range(len(regions_sorted)):
    #net1 = network_names[region_names.index(regions_sorted[i][0])]
    net1_index = network_index[region_names.index(regions_sorted[i][0])]
    worksheet.write(i, 0, label = regions_sorted[i][0])
    worksheet.write(i, 1, label = regions_sorted[i][1])
    #worksheet.write(i, 2, label = net1)
    worksheet.write(i, 3, label = net1_index)

worksheet = workbook.add_sheet('region_weight2_aal')#####################################
for i in range(len(regions_sorted2)):
    #net2 = network_names[region_names.index(regions_sorted2[i][0])]
    net2_index = network_index[region_names.index(regions_sorted2[i][0])]
    worksheet.write(i, 0, label = regions_sorted2[i][0])
    worksheet.write(i, 1, label = regions_sorted2[i][1])
    #worksheet.write(i, 2, label = net2)
    worksheet.write(i, 3, label = net2_index)
...
for i in range(len(feature_sorted)):
    ...
    worksheet.write(i, 5, label = network_index[index1])
    worksheet.write(i, 6, label = network_index[index2]) 
...
#import xlrd
#aal_file = r'D:\FMRI_ROOT\YIYU\ROIs\AAL\AAL2.xls'
#data = xlrd.open_workbook(aal_file)
#table = data.sheets()[0]          #通过索引顺序获取
##table = data.sheet_by_index(0) #通过索引顺序获取
...
#node_coords=[] 
#node_size = []   
## node_size调节，最大值为7，因为7左右软件中显示出来比较好看
#   
#for i in range(len(regions_sorted)):
...
#    str2 = str2 + '\n'
#
#file_object = open(os.path.join(save_root,r'regions_func_aal.node'), 'w')#######################################
#file_object.write(str2)
#file_object.close( )
#
#"""
...
#    str2 = str2 + '\n'
#
#file_object = open(os.path.join(save_root,r'regions2_func_aal.node'), 'w')#######################################
#file_object.write(str2)
#file_object.close( )
#
#
...
#
#"""
#生成脑区对应的连接信息 for brainnetviewer
#"""
#sorted_region_list = []
#sorted_region_intensity = []
#lens = len(regions_sorted)
...
#    strs = strs + '\n'
#
#file_object = open(os.path.join(save_root,r'aal_regions_links.edge'), 'w')  ####################
#file_object.write(strs)
#file_object.close( ) 
