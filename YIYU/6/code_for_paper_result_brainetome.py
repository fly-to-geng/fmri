...
#Ks = np.linspace(150,200,21).astype(int)
...
cv = LeavePOut(y.shape[0], p=1)
sk_f = SelectKBest(my_advance,k=230)
score_linear_svc = []
feaure_indexs = []
...
file_object = open(os.path.join(root,'data_for_brainnetviewer',r'regions_func_brainnetome.node'), 'w')#######################################
file_object.write(str2)
file_object.close( )
...
file_object = open(os.path.join(root,'data_for_brainnetviewer',r'brainnetome_regions_links.edge'), 'w')  ####################
file_object.write(strs)
file_object.close( ) 
