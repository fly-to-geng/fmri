...
Ks = np.linspace(10,1000,10).astype(int)
#Ks = np.linspace(150,200,25).astype(int)
...
# 选出来特征选择 204 个， 分类器选择线性linear_svm， 计算特征排序  准确率80.85%

cv = LeavePOut(y.shape[0], p=1)
sk_f = SelectKBest(my_advance,k=890)
score_linear_svc = []
feaure_indexs = []
...
file_object = open(os.path.join(root,'data_for_brainnetviewer',r'regions_func_aal.node'), 'w')#######################################
file_object.write(str2)
file_object.close( )
...
file_object = open(os.path.join(root,'data_for_brainnetviewer',r'aal_regions_links.edge'), 'w')  ####################
file_object.write(strs)
file_object.close( ) 
