...
root = r'D:\FMRI_ROOT\YANTAI\CONN\conn_project_2classz_func_brainnetome\classresult'
###############################################################################
"""
...
X_path = r'features_brainnetome_noise.mat'
...
features = features['features_brainnetome']
...
from sklearn.cross_validation import StratifiedKFold
#cv = StratifiedKFold(y, 72)

Ks = np.linspace(1,1000,10).astype(int)
#Ks = np.array( [303,304,305,306,307,308,309] )
...
# 选出来特征选择 304 个， 分类器选择线性LR， 计算特征排序  65.27%
# ==========================================================================
svc_linear =  SVC(C=1,kernel="linear")
cv = LeavePOut(y.shape[0], p=1)
sk_f = SelectKBest(f_classif,k=4)
score_linear_svc = []
feaure_indexs = []
for train,test in cv:  
    XX_train = sk_f.fit_transform(X[train],y[train])
...
node_size_end = [x*40 for x in node_size ]     
...
file_object = open(root+r'\Paper\result\brainnetome_function_regions.txt', 'w')
...
workbook.save(r'Paper\result\brainnetome_function_regions.xls') 
...
file_object = open(root + r'\Paper\result\brainnetome_function_links.txt', 'w')
...
#sio.savemat('selected_feature_brainnetome',{'selected_feature':a})
