...
save_path = r'D:\FMRI_ROOT\YIYU\MVPA'
n_folds = 2
...
def cross_validation_me(svc,X,y,p):
    from sklearn.cross_validation import LeavePOut  
    import numpy as np
    cv = LeavePOut(X.shape[0], p=p)
    cv_scores = []
...
n_step = 8
# 特征选择
Ks = np.linspace(1,500,n_step).astype(int)
scores_linear_svc = []
scores_rbf_svc = []
scores_nkc = []
feature_sorted_Ks = []

#predicted_linear = cross_val_predict(svc_linear, X,yyy, cv=n_folds)
...
for i in xrange(Ks.shape[0]):
    ...
    #scores_linear_svc.append( metrics.accuracy_score(y, predicted_linear) )
    scores_linear_svc.append( cross_validation_me(svc_linear,X_transformed,y,2) )
    ...
    #for i in xrange(feature_score.shape[0]):
        #result_dict[brain_region_names[i][0]] =  feature_score[i]
