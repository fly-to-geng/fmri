...
"""
X = features_functional['subjects_features_mat']
y = labels
brain_map_names = features_functional_names['brain_map_names'][0]
##############################################################################
"""
初步预处理，去掉方差为0的特征
"""
...
rfecv = RFECV(estimator=svc_linear, step=step, cv=StratifiedKFold(y, n_folds = n_folds),scoring='accuracy')
X_rfecv = rfecv.fit_transform(X, y)

lr = linear_model.LogisticRegression(C=100, penalty='l2', tol=0.0001)
predicted_rbf = cross_val_predict(lr, X_rfecv,y, cv=n_folds)
print metrics.accuracy_score(y, predicted_rbf)
# 获得特征排名
